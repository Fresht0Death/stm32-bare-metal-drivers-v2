...
//	 enable global interrupts
	...
	while(1)
	{
		...
		if(timer_started == 0)
		{
			adc1_pa0_single_polling_timed_init(5000, &TIMER_2);
			printf("Timer 2 and adc1 init finished\r\n");
			timer_started = 1;
		}

		timer_status_flag = check_timer_flag(&TIMER_2);

		if(timer_status_flag == 1)
		{

			raw_value = read_adc1_pa0_value();
			printf("ADC1 Raw Value: %ld\r\n", raw_value);
			timer_started = 0;

	    	voltage_integer_part = adc_to_voltage5V_integer_part(raw_value);
	    	voltage_decimal_part = adc_to_voltage5V_decimal_part(raw_value);
